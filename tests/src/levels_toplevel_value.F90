...
integer function do_it(lu,file) result(status)
  ...
  if(ier.lt.0)then
     print*,'ERROR with fstlir on PX for ip1,file ',prm%ip1,file
     return
  endif  

  if(abs(lev(10,10,1)/100.-px(10,10))>epsilon)then
     print*,'ERROR difference in pressure to high',lev(10,10,1)/100.,' VS',px(10,10)
     return
  else
