...
program tests
  ...
  nullify(ip1_list,dpidpis_cube,coef_b)
  ...
  do k=1,size(coef_b)
     ...
     if(coef_b(k).le.epsilon(coef_b(k)))then
        if(abs(coef_b(k)-dpidpis_cube(i0,j0,k))>epsilon(coef_b(k)))then
           print*,'ERROR 1 dpidpis_cube do not validate '
           OK=.false.
           exit
        endif
     else
        if(abs(dpidpis_cube(i0,j0,k)- coef_b(k))/coef_b(k)>eps)then
           print*,'ERROR 2 dpidpis_cube do not validate '
           OK=.false.
           exit
        endif
     endif
  ...
  do k=1,size(coef_b)
     ...
     if(coef_b(k).le.epsilon(coef_b(k)))then
        if(abs(coef_b(k)-dpidpis_cube_8(i0,j0,k))>epsilon(coef_b(k)))then
           print*,'ERROR 3 dpidpis_cube_8 do not validate'
           OK=.false.
           exit
        endif
     else
        if(abs(dpidpis_cube(i0,j0,k)- coef_b(k))/coef_b(k)>eps)then
           print*,'ERROR 4 dpidpis_cube_8 do not validate'
           OK=.false.
           exit
        endif
     endif
