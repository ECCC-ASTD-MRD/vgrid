...
program constructor
  ...
  stat = vgd_create_from_hyb_5001(vgdid,hyb=hyb,rcoef1=rcoef1,ptop_8=ptop,pref_8=pref)
  if(stat.ne.VGD_ERROR)OK=.false.
  print*,''
  print*,'===== Test on Error last hyb not equal 1.0, error message is normal ===='  
  stat = vgd_create_from_hyb_5001(vgdid,hyb=hyb(1:size(hyb)-1),rcoef1=rcoef1,ptop_8=ptop,pref_8=pref)
  if(stat.ne.VGD_ERROR)OK=.false.
  print*,''
  print*,'===== hyb not monotone, error message is normal ===='  
  hyb2=hyb
  hyb2(size(hyb)-3)=hyb(size(hyb)-2) 
  stat = vgd_create_from_hyb_5001(vgdid,hyb=hyb2,rcoef1=rcoef1,ptop_8=ptop,pref_8=pref)
  if(stat.ne.VGD_ERROR)OK=.false.
  ...
  stat = vgd_create_from_hyb_5001(vgdid,hyb=hyb(10:28),rcoef1=rcoef1,ptop_8=ptop,pref_8=pref)
  stat = vgd_get(vgdid,key='CB_M - vertical B coefficient',value=b_m_8)
  stat = vgd_get(vgdid,key='CA_M - vertical A coefficient',value=a_m_8)
