...
extern "C" void c_use_new_read() {
  ...
  if( my_vgd.Cvgd_new_read(vgd_p, iun, -1, -1, -1, -1) == VGD_ERROR ) {
    printf("ERROR with Cvgd_new_read on iun\n");
    return;
  }
  if( my_vgd.Cvgd_get_int_1d(vgd_p, "VIPT", &i_val, NULL, quiet) ==  VGD_ERROR ) {
    printf("ERROR with Cvgd_get_int for VIPT\n");
    return;
  }
  if( my_vgd.Cvgd_get_float_1d(vgd_p, "VCDT", &f_val, NULL , quiet) ==  VGD_ERROR ) {
    printf("ERROR with Cvgd_get_float_1d for VCDT\n");
    return;
  }
  if( my_vgd.Cvgd_get_double_1d(vgd_p, "CA_T", &a_8_t, NULL, quiet) ==  VGD_ERROR ) {
    printf("ERROR with Cvgd_get_double_1d for CA_T\n");
    return;
  }
  if( my_vgd.Cvgd_get_double_1d(vgd_p, "CB_T", &b_8_t, &nt, quiet) ==  VGD_ERROR ) {
    printf("ERROR with Cvgd_get_double_1d for CB_T\n");
    return;
  }

  // Size of thermo may also be obtained by this:
  ier = my_vgd.Cvgd_get_int(vgd_p, "NL_T", &nl_t, quiet);
  if(nl_t != nt ) {
  ...
  if ( my_vgd.Cvgd_get_double_3d(vgd_p, "VTBL", &table, &ni, &nj, &nk, quiet) ==  VGD_ERROR ) {
  ...
  if( my_vgd.Cvgd_write_desc(vgd_p, iun2) == VGD_ERROR ){
  ...
  ier = my_vgd.Cvgd_diag_withref_8(vgd_p, ni2, nj2, nl_t, i_val, levels_8, p0_8, in_log, dpidpis);
  if(ier == VGD_ERROR){
    status=VGD_ERROR;
  }
  ...
  if( my_vgd2.Cvgd_new_read(vgd2_p, iun, -1, -1, -1, -1) == VGD_ERROR ) {
