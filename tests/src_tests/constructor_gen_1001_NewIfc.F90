...
program constructor
  ...
  use vGrid_Descriptors, only: vgd_get,VGD_ERROR, vgd_create_from_hyb_1001
  ...
  stat = vgd_create_from_hyb_1001(vgdid,hyb=sigma)
  ...
  stat = vgd_create_from_hyb_1001(vgdid,hyb=sigma(1:size(sigma)-1))
  ...
  stat = vgd_create_from_hyb_1001(vgdid,hyb=sigma2)
  if(stat.ne.VGD_ERROR)OK=.false.
  deallocate(a_m_8,a_t_8,b_m_8,b_t_8,vipm,vipt,vcdm)
  !stat = vgd_free(vgdid)

  call ut_report(OK,'Grid_Descriptors::vgd_create_from_hyb_1001 vertical generate initializer value')
