...
contains
   !
   !**s/r vgrid_genab_3001  - Generates GEM model vertical grid parameters A, B and hyb
   !                      for momentun and thermodymic levels
   !
   subroutine vgrid_genab_3001 (F_hybuser, F_rcoef, &
        F_am_8, F_bm_8, F_cm_8, F_at_8, F_bt_8, F_ct_8,&
        F_ip1_m, F_ip1_t, F_err, dhm, dht)
      
      implicit none
      !      
      ! Arguments
      integer, intent(out) :: F_err
      real, dimension(:), intent(in) :: F_hybuser               ! user specification for vertical layering hyb hgts
      real, dimension(:), intent(in) :: F_rcoef                 ! user specification for rcoef
      real*8, dimension(:), pointer :: F_am_8, F_bm_8, F_cm_8   ! model As, Bs and Cs on momentum levels
      real*8, dimension(:), pointer :: F_at_8, F_bt_8, F_ct_8   ! model As, Bs and Cs on thermodynamic levels
      ...
      wronghyb = .false.
      do k=2, NK
         if (F_hybuser(k).ge.F_hybuser(k-1)) wronghyb = .true.
      enddo
      if (wronghyb) then
      ...
      ! Compute ip1 values
      do k=1,nk+1
         call convip(F_ip1_m(k),hybm(k),3,2,'',.false.)
         call convip(F_ip1_t(k),hybt(k),3,2,'',.false.)
      enddo
    ...
    !     __________________________________________________________________
      return
   end subroutine vgrid_genab_3001
