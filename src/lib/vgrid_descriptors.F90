...
contains
   ...
   integer function new_read(self,unit,format,ip1,ip2,kind,version,quiet,datev,etiket,ip3) result(status)
      ...
      ! Local variables
      integer :: ni,nj,nk, istat, error, l_datev, l_ip1, l_ip2, l_ip3, l_kind, l_version, l_quiet
      ...
      case ('BIN')
         ...
         if (istat /= 0) then
            write(app_msg,*) 'new_read: unable to allocate table_8'
         ...
         if (error < 0) then
            write(app_msg,*) 'new_read: problem creating record information'
            call Lib_Log(APP_LIBVGRID,APP_ERROR,app_msg)       
            return
         endif
         ! Warn user on invalid input format specification
      case DEFAULT
         write(app_msg,*) 'new_read: invalid constructor format request ',trim(myformat)
         call Lib_Log(APP_LIBVGRID,APP_ERROR,app_msg)       
         return
      end select
...
!!! Write descriptors
   ...
   integer function get_int_1d(self,key,value,quiet) result(status)
      ...
      ! Internal variables
      integer :: nl_, istat, error, l_quiet
